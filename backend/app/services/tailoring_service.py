...
DATA_DIR = Path(
    os.getenv("DATA_DIR", Path(__file__).parent.parent.parent.parent / "data")
)
...
async def tailor_documents_for_job(
    ...
    pdf_service.markdown_to_pdf(
        cover_letter_text, str(cl_path), doc_type="cover_letter"
    )
