...
class LeverRouter(ATSRouter):
    ...
    async def submit_application(
        ...
        try:
            ...
            if not posting_id:
                logger.error(
                    "Could not extract posting ID from URL",
                    extra_fields={
            ...
            if profile.get("linkedin_url"):
                data["urls"].append(
                    {"label": "LinkedIn", "url": profile.get("linkedin_url")}
                )

            if profile.get("portfolio_url"):
                data["urls"].append(
                    {"label": "Portfolio", "url": profile.get("portfolio_url")}
                )

            if profile.get("github_url"):
                data["urls"].append(
                    {"label": "GitHub", "url": profile.get("github_url")}
                )
            ...
            if resume_path and os.path.exists(resume_path):
                with open(resume_path, "rb") as f:
                    ...
                    data["resume"] = (
                        resume_content.hex()
                    )  # Lever expects base64-encoded resume
