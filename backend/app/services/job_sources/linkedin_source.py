...
class LinkedInJobSource(JobSource):
    ...
    async def _get_access_token(self) -> Optional[str]:
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                logger.info("LinkedIn access token obtained", extra_fields={
                    "source": self.name,
            ...
            logger.error(f"Failed to get LinkedIn access token: {e}", extra_fields={
                "source": self.name,
                "error": str(e),
            })
            return None
    ...
    async def fetch_jobs(self) -> List[JobListing]:
        ...
        if not self.is_configured:
            logger.warning("LinkedIn source not configured", extra_fields={
                "source": self.name,
        ...
        try:
            logger.info("Fetching LinkedIn jobs", extra_fields={
                "source": self.name,
            })
            
            access_token = await self._get_access_token()
            if not access_token:
                logger.error("Failed to authenticate with LinkedIn", extra_fields={
                    "source": self.name,
                })
                return []
            ...
            logger.info("LinkedIn jobs API integration ready", extra_fields={
                "source": self.name,
            ...
            logger.error(f"Failed to fetch LinkedIn jobs: {e}", extra_fields={
                "source": self.name,
                "error": str(e),
            })
            return []
