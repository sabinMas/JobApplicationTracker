...
class JobSourceManager:
    ...
    async def fetch_all(self) -> Dict[str, List[JobListing]]:
        ...
        logger.info("Fetching jobs from all sources", extra_fields={
        ...
        if tasks:
            ...
            for source_name, fetch_result in zip(source_names, fetches):
                if isinstance(fetch_result, Exception):
                    ...
                    logger.info(f"Fetched {len(fetch_result)} jobs from {source_name}", extra_fields={
                        "source": source_name,
    ...
    async def sync_to_database(self, jobs_by_source: Dict[str, List[JobListing]]) -> Dict[str, int]:
        """
        ...
        logger.info("Syncing jobs to database", extra_fields={
        ...
        async with AsyncSessionLocal() as session:
            for source_name, job_listing in all_jobs:
                try:
                    ...
                    if job_listing.apply_url in existing_urls:
                        stats["duplicates"] += 1
                        logger.debug(f"Job already exists: {job_listing.title}", extra_fields={
                            "source": source_name,
                    ...
                    logger.debug(f"Added job: {job_listing.title}", extra_fields={
                        "source": source_name,
                    ...
                    stats["errors"] += 1
                    logger.error(f"Error adding job: {e}", extra_fields={
                        "source": source_name,
            ...
            try:
                ...
                logger.info("Synced jobs to database", extra_fields={
                    "added": stats["added"],
                    "duplicates": stats["duplicates"],
                    "errors": stats["errors"],
                })
            except Exception as e:
                logger.error(f"Failed to commit jobs: {e}", extra_fields={
                    "error": str(e),
                })
    ...
    async def sync(self) -> Dict[str, int]:
        ...
        try:
            ...
            logger.error(f"Sync failed: {e}", extra_fields={
                "error": str(e),
            })
            raise
